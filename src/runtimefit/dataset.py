    ...
    with Path(path).open(encoding="utf-8") as handle:
        for line_number, line in enumerate(handle, start=1):
            ...
            try:
                ...
                raise ConfigError(f"Invalid dataset JSON on line {line_number}: {exc.msg}") from exc
            prompt = row.get("prompt")
            if not isinstance(prompt, str) or not prompt:
                raise ConfigError(f"Dataset line {line_number} needs a non-empty prompt")
            match = row.get("match", "exact")
            if match not in {"exact", "contains"}:
                raise ConfigError(f"Dataset line {line_number}: match must be exact or contains")
